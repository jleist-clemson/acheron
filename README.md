...
| `GET` | `/events/stats` | Event counts grouped by type *(stub — TODO)* |
| `GET` | `/events/search` | Full-text search via Elasticsearch — pass `?q=` *(stub — TODO)* |
| `GET` | `/events/stats/realtime` | Redis-cached realtime summary *(stub — TODO)* |
